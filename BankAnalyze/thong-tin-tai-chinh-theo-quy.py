...
for crawl_url in tqdm(BANKS_URL):
    ...
    sleep(2)
    season_xpath = '//*[@id="litimetabs1"]/a'
    element = driver.find_element(By.XPATH, season_xpath)
    element.click()
    ...
    years = lines[0].split(' Quý ')
    years = [i.replace('Quý','').strip() for i in years]
...
transform_table = pd.DataFrame(columns=["season", "group", "value", "bank"])
lastest_season = transform_table['season'].max()
seasons_found = scraped_table.columns[2:]
for season in tqdm(seasons_found):
    if type(lastest_season)==str and not is_season_in(season):
        continue
    else:
        adding_table = scraped_table[['feature', str(season), 'bank']]
        adding_table.insert(0,'season_happend',[season]*len(adding_table))
        adding_table.columns = transform_table.columns
        transform_table = pd.concat([transform_table, adding_table],axis=0)
transform_table.to_csv('results/thong-tin-tai-chinh-theo-quy.csv', index=False)